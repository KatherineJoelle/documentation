...
> Note: Keep in mind that the sounds of your scene will be heard by players that are in hearing range but outside your scene. Please be considerate with neighboring scenes when setting volumes and volume curves.
...
The `AudioStream` component is on by default unless there are other sounds in your scene. To activate it use the `call` property, `AudioStream.call`.
